...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.Activity(name=str(len(bot.guilds)) + "サーバー", type=discord.ActivityType.competing))
    try:
        await bot.load_extension("jishaku")
        print("jishakuを読み込みました\n")
    except Exception as e:
        print('jishakuの読み込み中にエラーが発生しました: ',"".join(traceback.format_exception(e)))
    try:
        print("スラッシュコマンドを読み込み中...\n")
        await load_extension()
        print("スラッシュコマンドを読み込みました\n")   
    except Exception as e:
        print('スラッシュコマンドの読み込み中にエラーが発生しました: ',"".join(traceback.format_exception(e)))
    ...
    try:
        ...
        print('スラッシュコマンドの同期中にエラーが発生しました: ',"".join(traceback.format_exception(e)))
...
async def load_extension():
    for cog in INITIAL_EXTENSIONS:
        try:
            ...
            print(f"{cog}の読み込み中にエラーが発生しました: ","".join(traceback.format_exception(e)))
